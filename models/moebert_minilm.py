...
from modules.modeling_moebert_gate import BertPreTrainedModel, MoEBertModel
...
class MoEBertMiniLM(BertPreTrainedModel):
    ...
    def forward(self, inputs):
        ...
        hidden_states, all_load_balances = self.bert(text_indices, attention_mask=text_mask, token_type_ids=text_segments)[:2]
        ...
        attn_load_balance = torch.stack(all_load_balances[0::2], 0)
        ffn_load_balance = torch.stack(all_load_balances[1::2], 0)
        if attn_load_balance.shape[1] == 1:
            load_balance = ffn_load_balance.sum(1).mean(0)
        else:
            load_balance = (attn_load_balance.sum(1).mean(0) + ffn_load_balance.sum(1).mean(0)) / 2
    ...
    def loss_fn(t_output, s_output):
        ...
        loss = loss / 3.0
        loss += 0.01 * s_output.load_balance
        return loss